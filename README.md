# PRESSMARK — Qikink Storefront

A 3D, print-on-demand storefront that connects to your Qikink catalog, takes secure payments through Razorpay, stores ratings/orders in Firebase (your Google account), and lives on GitHub so you can keep editing it.

**How the pieces fit together:**

```
 Shopper's browser                 GitHub Pages                Firebase (Google Cloud)          Qikink & Razorpay
 ┌────────────────┐   serves     ┌───────────────┐  calls    ┌────────────────────┐   calls   ┌──────────────────┐
 │ index.html      │◄────────────│ your GitHub    │──────────►│ Cloud Functions     │──────────►│ Qikink API        │
...
**Why GitHub can't be the whole server:** GitHub Pages only serves static files. Payment logic and your Qikink secret key have to run somewhere that keeps them hidden from the browser — that's what the small `functions/` folder does, hosted on Firebase (still free-tier friendly, still fully yours). GitHub still hosts and version-controls everything, including that backend code, and you redeploy it with one command whenever you edit it.
...
6. Cloud Functions need the **Blaze (pay-as-you-go)** plan because they call external APIs (Razorpay, Qikink). It has a generous free tier — you won't be billed at low volume, but Google requires a card on file. Upgrade under **Project settings → Usage and billing**.
...
1. Log into [dashboard.qikink.com](https://dashboard.qikink.com), and open the **API / Integration** section (Settings → API in the dashboard) — this is where Qikink issues your Client ID and Client Secret for programmatic access.
2. Note the API base URL they give you (sandbox vs. live).
3. Set these on your backend:
   ```bash
   firebase functions:config:set qikink.client_id="YOUR_CLIENT_ID" qikink.client_secret="YOUR_CLIENT_SECRET" qikink.api_base="https://sandbox.qikink.com/api"
   ```
4. Open `functions/index.js` and check the `createQikinkOrder` and `getQikinkProducts` functions — adjust the endpoint paths and payload fields to exactly match what Qikink's API docs specify (their field names may differ slightly from the placeholders here; the dashboard's API section includes a reference/Swagger doc).
5. Once confirmed working, replace the hardcoded `PRODUCTS` array in `index.html` with a `fetch()` to your deployed `getQikinkProducts` function, so the catalog stays in sync with Qikink automatically.
...
- [ ] Qikink API base switched from sandbox to production
...
|---|---|---|
| Products | 6 placeholder items in `index.html` | Point at `getQikinkProducts` once your Qikink key is set |
...
| Order fulfillment | Calls Qikink on successful payment or COD placement | Confirm field names match Qikink's docs (Step 3.4), including the `payment_method` field used for COD |
...
├─ functions/
│  ├─ index.js                 secure backend: Razorpay orders, Qikink orders, rating aggregation
│  └─ package.json
