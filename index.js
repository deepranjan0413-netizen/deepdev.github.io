 ...
 * it cannot safely hold secret API keys (Razorpay key_secret, Qikink
 ...
 *     qikink.client_id="xxx" qikink.client_secret="xxx" \
 *     qikink.api_base="https://sandbox.qikink.com/api"   // switch to live base URL when ready
 *
 ...
 * and forwards it to Qikink for fulfillment.
 * ---------------------------------------------------------------------- */
exports.verifyPayment = functions.https.onRequest((req, res) => {
  cors(req, res, async () => {
    try {
      ...
      // Push the fulfilment order to Qikink. Wrapped in try/catch so a
      // Qikink hiccup doesn't undo a successful, already-verified payment —
      // it's logged for manual retry instead.
      try {
        const qikinkResult = await createQikinkOrder({ cart, customer, razorpay_order_id });
        await db.collection("orders").doc(razorpay_order_id).update({
          qikinkStatus: "sent",
          qikinkResponse: qikinkResult,
        });
      } catch (qErr) {
        console.error("Qikink fulfilment error", qErr);
        await db.collection("orders").doc(razorpay_order_id).update({
          qikinkStatus: "failed",
          qikinkError: String(qErr),
        });
      }
 ...
 * No payment gateway involved — the order goes straight to Qikink marked
...
exports.createCodOrder = functions.https.onRequest((req, res) => {
  cors(req, res, async () => {
    try {
      ...
      try {
        const qikinkResult = await createQikinkOrder({
          cart,
          customer,
          razorpay_order_id: orderRef.id,
          paymentMethod: "cod",
        });
        await orderRef.update({ qikinkStatus: "sent", qikinkResponse: qikinkResult });
      } catch (qErr) {
        console.error("Qikink COD fulfilment error", qErr);
        await orderRef.update({ qikinkStatus: "failed", qikinkError: String(qErr) });
        // Order is still recorded — surface as failure so the buyer can retry
        // or you can manually push it to Qikink from the Firestore console.
        return res.status(502).json({ error: "Order saved but Qikink dispatch failed — check Firestore." });
      }
...
/* -------------------------------------------------------------------------
 * createQikinkOrder — internal helper
 * Adjust the endpoint path/payload to match the exact spec in your Qikink
 * dashboard's API/Integration docs (Dashboard > Settings > API) — in
 * particular, confirm the exact field name Qikink expects for COD vs
 * prepaid orders (commonly "payment_method": "cod" | "prepaid").
 * ---------------------------------------------------------------------- */
async function createQikinkOrder({ cart, customer, razorpay_order_id, paymentMethod = "prepaid" }) {
  const q = cfg().qikink || {};
  if (!q.client_id || !q.client_secret || !q.api_base) {
    throw new Error("Qikink credentials not configured — set them with firebase functions:config:set");
  }

  const payload = {
    order_number: razorpay_order_id,
    payment_method: paymentMethod, // "cod" or "prepaid" — verify against Qikink's docs
    customer,
  ...
  if (!resp.ok) {
    throw new Error(`Qikink API responded ${resp.status}: ${await resp.text()}`);
  }
  return resp.json();
}
...
/* -------------------------------------------------------------------------
 * getQikinkProducts
 * GET — proxies your Qikink product catalog so the frontend never needs
 * the Qikink secret key. Swap the mock PRODUCTS array in index.html for a
 * fetch() to this endpoint once it's deployed.
 * ---------------------------------------------------------------------- */
exports.getQikinkProducts = functions.https.onRequest((req, res) => {
  cors(req, res, async () => {
    try {
      const q = cfg().qikink || {};
      ...
      console.error("getQikinkProducts error", err);
